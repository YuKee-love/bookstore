...
public class BookDaoImpl extends BaseDao implements BookDao {
	...
	@Override
	public Page<Book> getPageByPrice(Page<Book> page, double minPirce, double maxPirce) {
		//查询价格区间的总记录条数
		String sql = "SELECT COUNT(1) FROM bs_book WHERE price BETWEEN ? AND ?";
		int count = getCount(sql, minPirce , maxPirce);
		//设置给page对象
		page.setTotalCount(count);
		//查询分页的价格区间的图书集合
		sql = "SELECT id , title , img_path imgPath, author , sales , "
				+ " stock ,price FROM bs_book WHERE price BETWEEN ? AND ? LIMIT ? , ?";
		List<Book> list = getBeanList(Book.class, sql, minPirce , maxPirce , page.getIndex() ,page.getSize() );
		page.setData(list);
		//返回设置所有数据的page对象
		return page;
	}
	...
	public List<Book> getBookList() {
		String sql = "SELECT id , title , img_path imgPath, author , sales , stock , "
				+ " price FROM bs_book";
