...
public class BookManagerServlet extends BaseServlet {
    ...
    //2、处理管理员删除指定图书的请求: 访问此方法  BookManagerServlet?type=deleteBook
    protected void deleteBook(HttpServletRequest request, HttpServletResponse response)
    	...
    	//获取 当前请求从哪个页面跳转过来的：referer地址
    	String referer = request.getHeader("referer");
    	//3、给用户响应：显示删除之后的所有图书列表
    	//如果需要使用request域共享数据使用 转发，如果不需要使用使用重定向
    	//跳转回删除之前的页面
    	//http://localhost:8080/BookStore_EG03/BookManagerServlet?type=findPage&pageNumber=2 只能让浏览器解析，转发不可以使用referer
	...
	/*
	 * protected void findAllBooks(HttpServletRequest request, HttpServletResponse
	 * response) throws ServletException, IOException { //调用其他类查询所有图书集合 List<Book>
	 * books = dao.getBookList(); //将图书集合共享到域中：request request.setAttribute("books",
	 * books); //转发到/pages/manager/book_manager.jsp 遍历显示图书集合
	 * request.getRequestDispatcher("/pages/manager/book_manager.jsp").forward(
	 * request, response); }
	 */
