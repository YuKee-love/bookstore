 ...
 * 		目的： 当访问Servlet时，服务器调用其声明周期方法，当调用到doGet时，我们希望进入父类的doGet方法控制子类方法的调用
 * 	2、访问BaseServlet的子类时，必须提供一个参数：type=要调用的方法名
 * 	3、以后子类Servlet中的处理请求的方法请求参数必须是：HttpServletRequest request, HttpServletResponse response
 * 	4、如果访问子类Servlet出现405的错误：
 * 		代表当前的请求方式在子类Servlet中没有实现doGet或doPost方法，被HttpServlet的doGet或doPost处理了请求
