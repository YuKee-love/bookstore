...
public class OrderServiceImpl implements OrderService {
	...
	public String createOrder(Cart cart, User user) {
		...
		//1.3 订单状态： 0 默认未发货
		int state = 0;
		Order order = new Order(orderId, cart.getTotalCount(), cart.getTotalAmount(), 
				createTime, state, user.getId());
		//1.4 保存订单到数据库
		orderDao.saveOrder(order);
		//2、将购物车的购物项集合转为 一个个的订单项对象，存到数据库并和 order关联
		List<CartItem> list = cart.getCartItemList();
		//遍历购物项集合
		for (CartItem cartItem : list) {
			Book book = cartItem.getBook();
			//一个购物项对应一个订单项
			OrderItem orderItem = new OrderItem(null, book.getTitle(), book.getAuthor(), 
					book.getImgPath(), book.getPrice(), cartItem.getCount(), cartItem.getAmount(), orderId);
			//将订单项存到数据库
